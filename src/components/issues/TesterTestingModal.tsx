        ...
        if (file) {
          ...
          const fileName =
            file.name && file.name !== "image.png" && file.name !== "blob"
              ? file.name
              : `qa-testing-evidence-${Date.now()}.${ext}`;
        ...
        <form onSubmit={handleSubmit} onPaste={handlePaste} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div>
            ...
            <div
              ...
              className="border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-pink-500 dark:hover:border-pink-500 rounded-2xl p-4 bg-slate-50/50 dark:bg-slate-950 transition flex flex-col sm:flex-row items-center justify-between gap-3"
            >
              ...
              <label className="cursor-pointer px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-200 shadow-sm flex items-center gap-1.5 transition shrink-0">
                {uploading ? <Loader2 className="h-3.5 w-3.5 animate-spin text-pink-600" /> : <Upload className="h-3.5 w-3.5 text-pink-600" />}
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        {isImg ? (
                          ...
                          <div className="h-8 w-8 rounded-lg bg-pink-50 dark:bg-pink-900/30 text-pink-600 flex items-center justify-center shrink-0">
